...
window.onload = () => {
    ...
    document.getElementById('kd').addEventListener('input', (event) => {
        gl.uniform1f(u_Kd, parseFloat(event.target.value));
        render();
    });

    document.getElementById('ks').addEventListener('input', (event) => {
        gl.uniform1f(u_Ks, parseFloat(event.target.value));
        render();
    });

    document.getElementById('shininess').addEventListener('input', (event) => {
        gl.uniform1f(u_Shininess, parseFloat(event.target.value));
        render();
    });

    document.getElementById('light-intensity').addEventListener('input', (event) => {
        const intensity = parseFloat(event.target.value);
        gl.uniform3fv(lightColorLoc, vec3(lightColor[0] * intensity, lightColor[1] * intensity, lightColor[2] * intensity)); 
        render();
    });
