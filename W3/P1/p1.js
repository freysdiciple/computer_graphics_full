...
window.onload = () => {
    ...
    let rotationMatrix = mult(rotate(35.264, vec3(1, 0, 0)), rotate(45, vec3(0, 1, 0))); 
    //35.264 is the angle according to wikipedia to get isometric view
    let lookAtMatrix = lookAt(vec3(0, 0, 1), vec3(0, 0, 0), vec3(0, 1, 0));
    let orthoMatrix = ortho(-1, 2, -1, 2, -1, 2);
    ...
    gl.uniformMatrix4fv(u_P, false, flatten(orthoMatrix));
    gl.uniformMatrix4fv(u_V, false, flatten(lookAtMatrix));
    gl.uniformMatrix4fv(u_M, false, flatten(rotationMatrix));
